...
void diffusion(data::Field const& s_old, data::Field const& s_new,
    ...
    #pragma omp parallel for default(none) shared(alpha,beta,s_old,s_new,f,iend,jend) collapse(2)
    ...
    {
        int i = nx - 1;
        #pragma omp parallel for default(none) shared(alpha,beta,s_old,s_new,f,jend,bndE,i)
        for (int j = 1; j < jend; j++) {
            f(i,j) = -(4. + alpha) * s_new(i,j)
                   + s_new(i-1,j) + bndE[j]
                   + s_new(i,j-1) + s_new(i,j+1)
    ...
    {
        int i = 0;
        #pragma omp parallel for default(none) shared(alpha,beta,s_old,s_new,f,jend,bndW,i)
        for (int j = 1; j < jend; j++) {
            f(i,j) = -(4. + alpha) * s_new(i,j)
                   + bndW[j]      + s_new(i+1,j)
                   + s_new(i,j-1) + s_new(i,j+1)
    ...
    // north boundary (plus NE and NW corners)
    {
        ...
        // north boundary
        #pragma omp parallel for default(none) shared(alpha,beta,s_old,s_new,f,iend,bndN,j)
        for (int i = 1; i < iend; i++) {
            f(i,j) = -(4. + alpha) * s_new(i,j)
                   + s_new(i-1,j) + s_new(i+1,j)
                   + s_new(i,j-1) + bndN[i]
    ...
    // south boundary (plus SW and SE corners)
    {
        ...
        // south boundary
        #pragma omp parallel for default(none) shared(alpha,beta,s_old,s_new,f,iend,bndS,j)
        for (int i = 1; i < iend; i++) {
            f(i,j) = -(4. + alpha) * s_new(i,j)
                   + s_new(i-1,j) + s_new(i+1,j)
                   + bndS[i]      + s_new(i,j+1)
