...
//all manage by the previous function
double hpc_norm2(Field const& x, const int N) {
    double result = 0;

    //TODO
    //taking advantage of the dot product function already implemented
...
void hpc_scaled_diff(Field& y, const double alpha, Field const& l,
    ...
    //TODO
    #pragma omp parallel for default(none) shared(alpha,y,N,l,r)
    for (int i = 0; i < N; i++){
        y[i] = alpha * (l[i]-r[i]);
    }
}
