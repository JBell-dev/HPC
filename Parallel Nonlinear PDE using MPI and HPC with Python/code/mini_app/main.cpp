...
void write_binary(std::string fname, Field& u, SubDomain& domain, Discretization& options) {
    std::string fname_bin = fname + ".bin";    // filename

    // we open the file as in example
    MPI_File filehandle;
    MPI_File_open(MPI_COMM_WORLD, fname_bin.c_str(), 
                  MPI_MODE_CREATE | MPI_MODE_WRONLY,
                  MPI_INFO_NULL, &filehandle);

    // subarray datatype for this process portion of the grid
    int sizes[2]    = {options.nx, options.nx};  // global grid size
    int subsizes[2] = {domain.nx, domain.ny};    // local grid size
    int starts[2]   = {domain.startx-1, domain.starty-1}; // starting points 
