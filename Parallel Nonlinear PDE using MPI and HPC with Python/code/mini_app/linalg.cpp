...
double hpc_dot(Field const& x, Field const& y) {
    ...
    //we need a global sum over all processes for the dot product
...
double hpc_norm2(Field const& x) {
    ...
    //we need a global sum over all processes for the norm
    double global_result = 0;
    MPI_Allreduce(&result, &global_result, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
    return sqrt(global_result);
}

// sets entries in a vector to value -> LOCAL ONLY
// x is a vector on length N
// value is a scalar
...
// computes y := alpha*x + y -> LOCAL ONLY
// x and y are vectors on length N
// alpha is a scalar
...
// computes y = x + alpha*(l-r) -> LOCAL ONLY
...
// computes y = alpha*(l-r) -> LOCAL ONLY
...
// computes linear combination of two vectors y := alpha*x + beta*z -> LOCAL ONLY
...
// copy one vector into another y := x -> LOCAL ONLY
