...
void write_binary(std::string fname, Field& u, SubDomain& domain, Discretization& options) {
    //we create a unique binary file for each rank
    std::string fname_bin = fname + "_rank" + std::to_string(domain.rank) + ".bin"; 
    MPI_File filehandle;
    MPI_File_open(MPI_COMM_SELF, fname_bin.c_str(), MPI_MODE_CREATE | MPI_MODE_WRONLY, MPI_INFO_NULL, &filehandle);

    //then we only write the data specific to each rank
    MPI_File_write_all(filehandle, u.data(), domain.nx * domain.ny, MPI_DOUBLE, MPI_STATUS_IGNORE);
    //we close the file
    MPI_File_close(&filehandle);
    //0 again writes the .bov but anyway we will need to change for the combined file after with the python scypt.
    if (domain.rank == 0) {
        std::string fname_bov = fname + ".bov";
        FILE* fp = fopen(fname_bov.c_str(), "w");
        fprintf(fp, "TIME: %f\n", options.nt * options.dt);
        fprintf(fp, "DATA_FILE: %s\n", fname_bin.c_str());
        fprintf(fp, "DATA_SIZE: %d %d %d\n", options.nx, options.nx, 1);
        ...
        fprintf(fp, "BRICK_SIZE: %f %f %f\n",
                (options.nx - 1) * options.dx, (options.nx - 1) * options.dx, 1.0);
        fclose(fp);
    }
}
...
int main(int argc, char* argv[]) {
    ...
    int rank, size; //we need to define through command line
